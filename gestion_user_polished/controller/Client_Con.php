...
class UserController
{
    ...
    private function buildUserFilters(array $filters): array
    {
        ...
        if (!empty($filters['agence']))    { $conditions[] = "ag.id_agence = :agence"; $params['agence'] = (int)$filters['agence']; }
    ...
    public function login(string $email, string $password): array
    {
        ...
        try {
            ...
            $stmt_attempts = $db->prepare("SELECT COUNT(*) as nb FROM login_attempts WHERE ip = ? AND statut = 'failed' AND created_at > DATE_SUB(NOW(), INTERVAL 15 MINUTE)");
            $stmt_attempts->execute([$ip]);
            $nb_failed = (int)$stmt_attempts->fetch()['nb'];
            if ($nb_failed >= 3) {
                $stmt_reste = $db->prepare("SELECT TIMESTAMPDIFF(SECOND, NOW(), DATE_ADD(MIN(created_at), INTERVAL 15 MINUTE)) as reste FROM login_attempts WHERE ip = ? AND statut = 'failed' AND created_at > DATE_SUB(NOW(), INTERVAL 15 MINUTE)");
            ...
            $user = $stmt->fetch();
            if (!$user) {
                $db->prepare("INSERT INTO login_attempts (email,ip,user_agent,statut) VALUES (?,?,?,'failed')")->execute([$email,$ip,$_SERVER['HTTP_USER_AGENT']??'']);
            ...
            return ['success'=>true, 'otp_required'=>true, 'role'=>$user['role']];
        } catch(Exception $e){ 
    ...
    public function verifyOTP(string $code): array
    {
        ...
        try {
            ...
            $db->prepare("UPDATE user SET last_login=NOW() WHERE id_user=:id")->execute(['id'=>$id]);
            ...
            if ($email_log) {
                $db->prepare("INSERT INTO login_attempts (email,ip,user_agent,statut) VALUES (?,?,?,'success')")
                   ->execute([$email_log, $ip_log, $_SERVER['HTTP_USER_AGENT']??'']);
                // Nettoyer les tentatives échouées de cette IP
                $db->prepare("DELETE FROM login_attempts WHERE ip=? AND statut='failed'")->execute([$ip_log]);
            }
            
            unset($_SESSION['otp_user_id'],$_SESSION['otp_role'],$_SESSION['otp_nom'],$_SESSION['otp_email']);
    ...
    public function findOrCreateGithubUser(array $githubUser): array
    {
        ...
        try {
            $db = config::getConnexion();
            $stmt = $db->prepare("SELECT id_user, nom, role, statut FROM user WHERE github_id = :gid OR (email = :email AND email != '') LIMIT 1");
    ...
    public function addclient(User $user, ?string $referred_by = null, ?int $id_agence = null): void
    {
        ...
        $db->prepare("INSERT INTO user (nom,prenom,email,mot_de_passe,telephone,cin,adresse,date_naissance,role,statut,date_creation,google_id,avatar_url,referral_code) 
                     VALUES (:nom,:prenom,:email,:mdp,:tel,:cin,:adr,:dn,'client','actif',NOW(),:gid,:avurl,:ref)")
           ->execute([
               ...
               'dn'=>$user->getDateNaissance()?$user->getDateNaissance()->format('Y-m-d'):null,
        ...
        if ($referred_by) {
            $stmt = $db->prepare("UPDATE user SET points_parrainage = points_parrainage + 50 WHERE referral_code = ?");
        ...
        $db->prepare("UPDATE user SET last_login=NOW() WHERE id_user=:id")->execute(['id'=>$uid]);
    ...
    public function addUserAdmin(
        ...
        $db = config::getConnexion();
        $db->prepare("INSERT INTO user (nom,prenom,email,mot_de_passe,telephone,cin,role,statut,date_creation) VALUES (:nom,:prenom,:email,:pw,:tel,:cin,:role,:statut,NOW())")
    ...
    public function getAllUsers(int $page=1, int $perPage=20): array
    {
        ...
        if ($this->isAdminAgence() && isset($_SESSION['id_agence'])) {
            $sql.=" WHERE (ag.id_agence=:ag OR a.id_agence=:ag2 OR c.id_agence=:ag3)";
    ...
    public function countAllUsers(): int
    {
        $db=config::getConnexion();
        if ($this->isAdminAgence() && isset($_SESSION['id_agence'])) {
            $s=$db->prepare("SELECT COUNT(*) FROM user u LEFT JOIN agent ag ON u.id_user=ag.id_user LEFT JOIN admin a ON u.id_user=a.id_user LEFT JOIN client c ON u.id_user=c.id_user WHERE ag.id_agence=:ag OR a.id_agence=:ag2 OR c.id_agence=:ag3");
    ...
    {
        $stmt=config::getConnexion()->prepare($this->baseUserSelect()." WHERE u.id_user=:id");
        $stmt->execute(['id'=>$id]);
    ...
    public function updateClient(int $id_user, string $nom, string $prenom, string $email, ?string $telephone=null, ?string $adresse=null, ?string $date_naissance=null): void
    {
        ...
        config::getConnexion()->prepare("UPDATE user SET nom=:n,prenom=:p,email=:e,telephone=:t,adresse=:a,date_naissance=:dn WHERE id_user=:id")
    ...
    public function getAdminProfile(int $id_user): ?array
    {
        $stmt=config::getConnexion()->prepare(
            "SELECT u.nom,u.prenom,u.email,u.telephone,u.cin,u.role,u.statut,u.avatar,u.date_creation,u.face_encoding,
    ...
    public function getStats(?int $days=null): array
    {
        ...
        if ($this->isAdminAgence()&&isset($_SESSION['id_agence'])) {
            ...
            $params['ag']=(int)$_SESSION['id_agence']; $params['ag2']=(int)$_SESSION['id_agence']; $params['ag3']=(int)$_SESSION['id_agence'];
    ...
    public function getAdvancedStats(?int $days=null): array
    {
        ...
        if ($this->isAdminAgence() && ($agId = $this->getSessionAgence())) {
            $agencyFilter = " LEFT JOIN agent ag ON u.id_user=ag.id_user LEFT JOIN admin a ON u.id_user=a.id_user LEFT JOIN client c ON u.id_user=c.id_user WHERE (ag.id_agence=:ag OR a.id_agence=:ag2 OR c.id_agence=:ag3)";
    ...
    public function getSocialData(int $my_id): array {
        ...
        // Amis (avec is_online calculé SQL)
        $stmt = $db->prepare("
