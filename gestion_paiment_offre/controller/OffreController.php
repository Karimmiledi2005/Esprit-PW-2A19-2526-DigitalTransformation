...
{
    private string $baseUrl = '/projet_web/MVC%20TEMPLATE/controller/OffreController.php';
            ...
            if (empty($errors)) {
                $offre = new Offre(
                    ...
                    (isset($_POST['plafond']) && $_POST['plafond'] !== '')
                        ? round((float)$_POST['plafond'], 3) : null,
                    (int)$_POST['duree_min'],
                    $_POST['statut'] ?? 'active'
                );
                ...
                try {
                    $stmt = $db->prepare("
                        INSERT INTO offre
                            (nom_offre, type_offre, description, prix_mensuel,
                             prix_annuel, couverture, plafond, duree_min, statut)
                        VALUES
                            (:nom_offre, :type_offre, :description, :prix_mensuel,
                             :prix_annuel, :couverture, :plafond, :duree_min, :statut)
                    ");
    ...
    public function modifier(): void
    {
        ...
        if (!$offre) $this->redirectToIndex('', 'Offre introuvable');
    ...
    public function supprimer(): void
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            try {
                ...
                if ($count > 0) {
                    $db->prepare("UPDATE offre SET statut = 'archivee' WHERE id_offre = ?")
                       ->execute([$id]);
                    $this->redirectToIndex('Offre archivée (paiements liés)');
                } else {
                    $db->prepare("DELETE FROM offre WHERE id_offre = ?")
                       ->execute([$id]);
    ...
    public function changerStatut(): void
    {
        ...
        if ($id <= 0)                                   $this->redirectToIndex('', 'Identifiant invalide');
        if (!in_array($statut, $statuts_valides, true)) $this->redirectToIndex('', 'Statut invalide');

        $db   = config::getConnexion();
        $stmt = $db->prepare("SELECT id_offre FROM offre WHERE id_offre = ?");
        $stmt->execute([$id]);
        if (!$stmt->fetch())                            $this->redirectToIndex('', 'Offre introuvable');

        try {
            $db->prepare("UPDATE offre SET statut = ? WHERE id_offre = ?")
               ->execute([$statut, $id]);
            $labels = ['active'=>'activée','suspendue'=>'suspendue','archivee'=>'archivée'];
    ...
    private function valider(array $data): array
    {
        ...
        if (empty($errors['prix_mensuel']) && empty($errors['prix_annuel'])
            && is_numeric($prix_mensuel)   && is_numeric($prix_annuel)) {
