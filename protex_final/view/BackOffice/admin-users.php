    ...
    .then(data => {
        ...
        modalBody.innerHTML = `
            <div style="display:flex;align-items:center;gap:16px;margin-bottom:24px;padding-bottom:20px;border-bottom:1px solid var(--glass-border);">
                ...
                <div>
                    ...
                    <div style="margin-top:8px;display:flex;gap:8px">
                        ...
                        <span class="badge badge-${u.statut}">${u.statut}</span>
                    </div>
                </div>
            </div>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:20px;">
                ...
                ${field('bi-star','Points', (u.points_parrainage || 0) + ' pts')}
            ...
            <div style="margin-top:20px;">
                ...
                ${data.logins.length ? data.logins.map(l => `
                    <div style="display:flex; justify-content:space-between; font-size:12px; padding:6px 0; border-bottom:1px solid rgba(255,255,255,0.03);">
                        ...
                        <span style="color:${l.statut==='success'?'#2ed573':'#ff4757'}">${l.statut}</span>
...
.then(profileData => {
    ...
    // Mettre à jour l'avatar et les infos dans le sidebar
    const initiales = ((profileData.nom||'').charAt(0) + (profileData.prenom||'').charAt(0)).toUpperCase();
    const avatarValue = profileData.avatar || '';
    const skipSidebar = !avatarValue || avatarValue === 'default.png' || avatarValue === 'default' || avatarValue.trim() === '';
    const avatarPath = skipSidebar ? '' : (avatarValue.includes('/') ? avatarValue : '../../uploads/avatars/' + avatarValue);
    ...
    if (nameEl) nameEl.textContent = (profileData.prenom||'') + ' ' + (profileData.nom||'');
    const roleEl = document.querySelector('.user-role');
    if (roleEl && profileData.role) {
        let roleTxt = profileData.role.toLowerCase() === 'admin' ? 'Administrateur' : profileData.role.charAt(0).toUpperCase() + profileData.role.slice(1);
        if (profileData.nom_agence) {
            roleTxt += ' (' + profileData.nom_agence + ')';
        }
