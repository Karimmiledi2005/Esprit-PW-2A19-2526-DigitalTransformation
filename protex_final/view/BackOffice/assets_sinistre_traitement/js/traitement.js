...
async function loadFraudPanelTraitement(idSinistre) {
  ...
  try {
    ...
    if (json.success && json.data) {
      ...
      panel.querySelector('.fraud-flags').innerHTML = '<span style="color:var(--text-secondary);font-size:12px;">Aucune analyse. <button class=\"fraud-reanalyse-btn\" onclick=\"reanalyserFraudTraitement(' + idSinistre + ')\"><i class=\"bi bi-shield-shaded\"></i> Analyser</button></span>';
    }
  } catch(e) { /* silencieux */ }
}
      ...
      document.getElementById('viewContent').innerHTML =
        ...
        '<div class="fraud-panel" id="fraudPanel" style="display:none;">' +
          '<div class="fraud-panel-header">' +
            ...
            '<button class="fraud-reanalyse-btn" id="fraudReanalyseBtn"><i class="bi bi-arrow-repeat"></i> Relancer l\'analyse</button>' +
          '</div>' +
...
async function loadPermissions() {
  try {
    ...
    // Masquer le bouton Ajouter si pas autorisé (Agent non autorisé selon votre demande)
    const addBtn = document.querySelector('button[onclick="openCreateModal()"]');
    if (addBtn && !perms.canCreateTraitement) {
      addBtn.remove();
    }

    // Désactiver le choix de l'agent si ce n'est pas un admin (l'agent ne s'auto-assigne pas)
