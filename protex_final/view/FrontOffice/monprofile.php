...
<body>
        ...
        <main class="main">
            ...
            <div class="content">
                ...
                <div class="grid-2">
                        ...
                        <div class="card-body">
                            ...
                            <div style="position:relative; width:120px; height:120px; margin:0 auto 24px;">
                                ...
                                <div id="avatarPreview"
                                    style="width: 100%; height: 100%; border-radius: 50%; background: var(--glass-bg); border: 2px solid var(--accent); display: flex; align-items: center; justify-content: center; overflow: hidden; position: absolute; top: 0; left: 0; z-index: 1;">
                                </div>
                                <canvas id="avatar3D" width="120" height="120"
                                    style="border-radius:50%; cursor:pointer; position:absolute; z-index:2;"></canvas>
                                <canvas id="particleAvatar" width="120" height="120"
                                    style="border-radius:50%; position:absolute; top:0; left:0; pointer-events:none; z-index:3;"></canvas>
                            </div>
                            <div id="avatarMoodLabel" style="text-align:center; font-size:12px; font-weight:bold; margin-bottom:10px; color:var(--accent); min-height:18px;"></div>
                            <div id="ptx-badges"
                        ...
                        <div class="card-body">
                            <div class="info-item" style="margin-bottom:16px">
                                <p style="font-size:13px; color:var(--text-secondary)">Connectez-vous rapidement et en
                                    toute sécurité grâce à la reconnaissance faciale propulsée par Intelligence
                    ...
                    <!-- CARTE CONTACTS (Réseau Social) -->
                    <div class="card" style="margin-top:20px;">
                        <div class="card-header"
                            style="display:flex; justify-content:space-between; align-items:center;">
                            <div class="card-title"><i class="bi bi-people"
                                    style="color:var(--accent);margin-right:8px"></i>Mon Réseau <span id="networkCount"
                                    style="font-size:12px; opacity:0.6; font-weight:400;">(0 contacts)</span></div>
                            <div style="position:relative; width:150px;">
                                <i class="bi bi-search"
                                    style="position:absolute; left:10px; top:50%; transform:translateY(-50%); font-size:12px; color:var(--text-secondary);"></i>
                                <input type="text" id="networkSearch" placeholder="Filtrer..."
                                    style="width:100%; padding:6px 10px 6px 30px; background:var(--glass-bg); border:1px solid var(--glass-border); border-radius:20px; color:var(--text-primary); font-size:11px;">
                            </div>
                        </div>
                        <!-- Onglets filtrage réseau -->
                        <div style="display:flex; gap:4px; padding:10px 12px 0; background:rgba(0,0,0,0.15);">
                            <button onclick="filterNetwork('all')" id="netTab-all"
                                style="flex:1; padding:7px 4px; border:none; border-radius:8px; background:var(--accent); color:#fff; font-size:11px; font-weight:700; cursor:pointer; transition:0.3s;">Tous</button>
                            <button onclick="filterNetwork('online')" id="netTab-online"
                                style="flex:1; padding:7px 4px; border:none; border-radius:8px; background:rgba(255,255,255,0.05); color:var(--text-secondary); font-size:11px; font-weight:700; cursor:pointer; transition:0.3s;">&#128994;
                                En ligne</button>
                            <button onclick="filterNetwork('trusted')" id="netTab-trusted"
                                style="flex:1; padding:7px 4px; border:none; border-radius:8px; background:rgba(255,255,255,0.05); color:var(--text-secondary); font-size:11px; font-weight:700; cursor:pointer; transition:0.3s;">&#11088;
                                Confiance</button>
                            <button onclick="filterNetwork('pending')" id="netTab-pending"
                                style="flex:1; padding:7px 4px; border:none; border-radius:8px; background:rgba(255,255,255,0.05); color:var(--text-secondary); font-size:11px; font-weight:700; cursor:pointer; transition:0.3s; position:relative;">
                                Invitations
                                <span id="invitationBadge"
                                    style="position:absolute; top:-4px; right:-2px; background:#ff4757; color:#fff; border-radius:50%; width:16px; height:16px; font-size:9px; display:none; align-items:center; justify-content:center;"></span>
                            </button>
                            <button onclick="filterNetwork('sos')" id="netTab-sos"
                                style="flex:1; padding:7px 4px; border:none; border-radius:8px; background:rgba(255,255,255,0.05); color:var(--text-secondary); font-size:11px; font-weight:700; cursor:pointer; transition:0.3s;">
                                🆘 SOS
                            </button>
                        </div>
                        <div class="card-body" style="padding:0">
                            <div id="ptx-contacts-list" style="max-height: 400px; overflow-y: auto;">
                                <!-- Injected by JS -->
                            </div>

                            <!-- NOUVEAU PANNEAU SOS CENTRALIS -->
                            <div id="sos-panel" style="display:none; padding:20px; max-height:400px; overflow-y:auto;">
                                <div style="background:rgba(255,71,87,0.05); border:1px solid rgba(255,71,87,0.1); border-radius:15px; padding:15px; margin-bottom:20px;">
                                    <div style="font-weight:700; color:#ff4757; margin-bottom:5px; display:flex; align-items:center; gap:8px;">
                                        <i class="bi bi-shield-fill-exclamation"></i> Centre de Sécurité SOS
                                    </div>
                                    <p style="font-size:12px; color:var(--text-secondary); margin:0;">
                                        En cas d'urgence, vos contacts de confiance recevront votre position GPS et une alerte immédiate.
                                    </p>
                                </div>

                                <div style="margin-bottom:20px;">
                                    <div style="font-size:11px; text-transform:uppercase; color:var(--text-secondary); font-weight:700; margin-bottom:10px; display:flex; justify-content:space-between;">
                                        <span>⭐ Mes contacts de confiance</span>
                                        <span style="color:var(--accent); cursor:pointer; font-size:10px;" onclick="filterNetwork('trusted')">Voir tout</span>
                                    </div>
                                    <div id="sosTrustedPreview" style="display:flex; gap:10px; overflow-x:auto; padding-bottom:5px;">
                                        <!-- Aperçu des contacts de confiance -->
                                    </div>
                                </div>

                                <button class="sos-btn-pulse" onclick="triggerSOS()" style="margin-bottom:25px;">
                                ...
                                <div style="border-top:1px solid var(--glass-border); padding-top:15px;">
                                    <div style="font-size:11px; text-transform:uppercase; color:rgba(255,71,87,0.8); font-weight:700; margin-bottom:8px;">
                                        <i class="bi bi-clock-history"></i> Historique SOS récent
                                    </div>
                                    <div id="sosHistoryList" style="font-size:12px; color:var(--text-secondary);">
        ...
        <div
            ...
            <div id="sosCountdownZone" style="margin-bottom:20px;">
                ...
                <button onclick="cancelSOSCountdown()"
                    ...
                    Annuler le compte à rebours</button>
            </div>
    ...
    <div id="faceIdSetupModal" class="setup-modal">
        <div class="setup-card">
            ...
            <div id="setupFaceStatus" class="setup-status">Prêt pour l'analyse...</div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            window.updateCompletionScore = function () {
                ...
                if (tip) {
                    if (pct < 100) tip.textContent = "Complétez votre profil pour gagner des points !";
                    else tip.innerHTML = "🎉 Profil 100% complété !";
                }
            };
            ...
            window.calculateBadges = function (data) {
                ...
                const badges = [
                    ...
                    { name: 'Protecteur', icon: '🛡️', min: 250, color: '#2ed573' },
            ...
            function loadProfile() {
                fetch('get_user.php')
                    ...
                    .then(data => {
                        ...
                        // Avatar dans le header et dropdown
                        document.querySelectorAll('.avatar-btn, .dropdown-avatar').forEach(el => {
                            if (avatarSrc) {
                                el.innerHTML = `<img src="${avatarSrc}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;" onerror="this.parentElement.textContent='${initiales}'">`;
                            } else {
                        ...
                        // Nom complet dans le dropdown
                        const dnEl = document.querySelector('.dropdown-name');
                        if (dnEl) dnEl.textContent = (data.prenom || '') + ' ' + (data.nom || '');
                        const deEl = document.querySelector('.dropdown-email');
                        if (deEl) deEl.textContent = data.email || '';

                        const drEl = document.querySelector('.dropdown-role');
                        if (drEl) {
                            let roleTxt = data.role === 'client' ? 'Client' : data.role;
                            if (data.nom_agence) {
                                roleTxt += ' (' + data.nom_agence + ')';
                            }
                            drEl.textContent = roleTxt;
                        }
            ...
            function getAvatarUrl(url) {
                ...
                // Si l'url contient déjà un chemin (ex: uploads/...), on le garde, sinon on préfixe
            ...
            if (savePwdBtn) {
                savePwdBtn.addEventListener('click', function () {
                    ...
                    if (p1.length < 8) {
                        showToast('Mot de passe trop court. 8 caractères min avec majuscule, minuscule, chiffre et caractère spécial', 'warning'); return;
                    }
                    if (p1 !== p2) {
                        showToast('Les mots de passe ne correspondent pas. Doit être identique au mot de passe', 'danger'); return;
                    }
            ...
            if (btnDeactivate) {
                btnDeactivate.addEventListener('click', function () {
                    if (confirm("⚠ Êtes-vous sûr de vouloir désactiver votre compte ? Cette action est irréversible sans l'intervention d'un administrateur.")) {
                        fetch('update_user.php', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            ...
                            .then(data => {
                                if (data.success) {
                                    alert("Votre compte a été désactivé. Vous allez être redirigé.");
                                    window.location.href = 'login.html';
                                } else {
            ...
            window.triggerSOS = function () {
                if (window.setAvatarMood) setAvatarMood('sos');
                ...
                if (trusted.length === 0) {
                    listEl.innerHTML = '<div style="color:#ff4757; font-size:12px; font-weight:700;">⚠️ Aucun contact de confiance !</div>';
                } else {
                    let names = [];
                ...
                if (navigator.geolocation) {
                    navigator.geolocation.getCurrentPosition(
                        ...
                        () => { document.getElementById('sosGPSStatus').innerHTML = '⚠️ GPS non disponible'; }
                    );
                }
            ...
            window.cancelSOS = function () {
                ...
                if (window.setAvatarMood) setAvatarMood('normal');
            };
            ...
            window.openChat = function (id, name, avatar, isOnline) {
                ...
                if (window.setAvatarMood) setAvatarMood('chat');
            };
            ...
            if (setupFaceIdBtn) {
                setupFaceIdBtn.addEventListener('click', async function () {
                    ...
                    try {
                        ...
                        setTimeout(() => {
                            ...
                            let interval = setInterval(() => {
                                ...
                                if (captures.length >= maxCaptures) {
                                    ...
                                    setTimeout(() => {
                                        setupStatus.textContent = "Analyse et entraînement...";
                        ...
                        setupStatus.textContent = 'Accès caméra refusé';
                        setupStatus.style.color = '#ff4757';
                        setTimeout(() => { stopSetupCamera(); }, 2500);
                    }
                });
    ...
    <script>
        ...
        function init3DAvatar() {
            ...
            const mat = new THREE.MeshPhongMaterial({
                color: 0x00b4d8, wireframe: true, emissive: 0x002244
            });
            ...
            function animate() {
                ...
                if (head) { head.rotation.y += 0.01; head.rotation.x += 0.005; }
        ...
        window.setAvatarMood = function (mood) {
            if (!head) return;
            const label = document.getElementById('avatarMoodLabel');
            if (mood === 'chat') {
                head.material.color.setHex(0xff6b1a);
                head.material.wireframe = false;
                if (label) label.textContent = "💬 En discussion";
            } else if (mood === 'sos') {
                head.material.color.setHex(0xff4757);
                head.material.wireframe = true;
                if (label) label.textContent = "🆘 ALERTE SOS";
            } else if (mood === 'happy') {
                head.material.color.setHex(0x2ed573);
                if (label) label.textContent = "😊 Joyeux";
            } else {
                head.material.color.setHex(0x00b4d8);
                head.material.wireframe = true;
                if (label) label.textContent = "🤖 Normal";
            }
        };

        window.explodeAvatar = function () {
            if (!head) return;
            let s = 1;
            const exp = setInterval(() => {
                s += 0.1; head.scale.set(s, s, s);
                if (s > 2) { clearInterval(exp); head.scale.set(1, 1, 1); }
            }, 50);
        };

        // --- PARTICULES ---
        const pCanvas = document.getElementById('particleAvatar');
        const pCtx = pCanvas ? pCanvas.getContext('2d') : null;
        let particles = [];
        if (pCanvas) {
            for (let i = 0; i < 20; i++) particles.push({ x: 60, y: 60, r: Math.random() * 2, a: Math.random() * Math.PI * 2, v: Math.random() + 0.5 });
            function drawP() {
                pCtx.clearRect(0, 0, 120, 120);
                pCtx.fillStyle = 'rgba(0,180,216,0.5)';
                particles.forEach(p => {
                    p.x += Math.cos(p.a) * p.v; p.y += Math.sin(p.a) * p.v;
                    if (p.x < 0 || p.x > 120 || p.y < 0 || p.y > 120) { p.x = 60; p.y = 60; }
