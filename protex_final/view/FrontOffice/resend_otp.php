...
try {
    ...
    echo json_encode(['success' => false, 'message' => 'Erreur envoi email', 'debug' => $e->getMessage()]);
}

} catch (Throwable $eGlobal) {
    ob_clean();
    error_log('resend_otp.php: ' . $eGlobal->getMessage());
    echo json_encode(['success' => false, 'message' => 'Erreur serveur', 'debug' => $eGlobal->getMessage()]);
}
