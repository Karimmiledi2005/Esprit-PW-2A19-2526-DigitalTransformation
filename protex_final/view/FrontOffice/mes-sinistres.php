    ...
    <main class="main">
        ...
        <div class="content">
            ...
            <div id="sinistreModal" class="modal-overlay">
                <div class="modal-box">
                    <h3 id="modalTitle">Déclarer un sinistre</h3>
                    <form id="sinistreFormElement" onsubmit="handleSinistreSubmit(event)">
                        ...
                        <div class="form-group animated-field">
                            ...
                            <textarea id="description" class="form-control" rows="3" placeholder="Décrivez le sinistre..."></textarea>
                        </div>
                        <div class="form-group animated-field">
...
async function selectSinistre(id) {
    ...
    document.getElementById('detailPanel').innerHTML = `
        <div class="sinistre-box">
            ...
            <p style="font-size:13px;color:var(--text-secondary);margin:10px 0 12px;">${s.description}</p>
            <img src="${s.photo}" style="width:100%;border-radius:10px;margin-bottom:14px;" alt="photo" onerror="this.style.display='none'">
            ...
            <div class="sinistre-title" style="margin-bottom:10px;">Suivi du statut</div>
            <div class="timeline" style="margin-bottom:16px;">
                <div class="timeline-item">
                    <div class="timeline-dot gray"></div>
                    <div class="timeline-content"><div class="timeline-title">Déclaration reçue</div><div class="timeline-date">${fmt(s.date)}</div></div>
                </div>
                ${s.statut !== 'en_attente' ? `
                <div class="timeline-item">
                    <div class="timeline-dot blue"></div>
                    <div class="timeline-content"><div class="timeline-title">En cours de traitement</div></div>
                </div>` : ''}
                ${s.statut === 'rembourse' ? `
                <div class="timeline-item">
                    <div class="timeline-dot green"></div>
                    <div class="timeline-content"><div class="timeline-title">Remboursé</div></div>
                </div>` : ''}
                ${s.statut === 'refuse' ? `
                <div class="timeline-item">
                    <div class="timeline-dot red"></div>
                    <div class="timeline-content"><div class="timeline-title">Refusé</div></div>
                </div>` : ''}
            </div>

            <div class="sinistre-title" style="margin-bottom:10px;">
...
async function handleSinistreSubmit(e) {
    ...
    try {
        ...
        if (editingId) {
            // ── MODIFIER un sinistre existant ──
            const body = new URLSearchParams({ id: editingId, id_contrat: contrat, type, description: desc });
            res  = await fetch('sinistre_update.php', { method:'POST', body });
            json = await res.json();
            if (json.success) {
                showToast('Sinistre modifié avec succès.', 'success');
                closeSinistreModal();
                await loadSinistres();
                if (selectedId === editingId) selectSinistre(editingId);
            } else {
