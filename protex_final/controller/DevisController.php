...
class DevisController
{
    ...
    {
        $this->baseUrl = defined('BASE_URL') ? BASE_URL : '/integration/protex_final';
    }
        ...
        try {
            $statsSql = "
                SELECT
                    ...
                    SUM(CASE WHEN statut = 'accepte' THEN montant_estime ELSE 0 END) AS total_accepte
                FROM devis
            ";
        ...
        try {
            $statsByType = $db->query("
                SELECT d.type_assurance,
                       ...
                       SUM(CASE WHEN d.statut = 'accepte' THEN 1 ELSE 0 END) AS acceptes,
...
<tr>
                ...
                try {
                    ...
                    if ($ancienStatut !== $nouveauStatut) {
                        try {
                            ...
                            $messageEmail = ' ⚠️ Erreur email : ' . $e->getMessage();
                        }
                    }
    ...
    public function convertirEnContrat(): void
    {
        ...
        try {
            ...
            $stmtCtr = $db->prepare("
                SELECT COUNT(*) FROM contrat WHERE id_devis = ?
            ");
            $stmtCtr->execute([$id]);
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                $db->prepare("
                    INSERT INTO contrat (numero_contrat, id_devis, id_categorie, type_contrat,
                        prime_contrat, date_debut_contrat, date_fin_contrat, statut_contrat, notes)
                    VALUES (:numero, :id_devis, :id_categorie, :type, :prime, :debut, :fin, 'actif', :notes)
                ")->execute([
                ...
                $db->prepare("UPDATE devis SET statut = 'en_cours' WHERE id_devis = ?")
