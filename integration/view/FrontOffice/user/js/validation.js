...
(function initRegisterForm() {
    ...
    form.addEventListener('submit', function (e) {
        ...
        if (nom && !validateField(nom, 'nom')) valid = false;
        if (prenom && !validateField(prenom, 'prenom')) valid = false;
        if (email && !validateField(email, 'email')) valid = false;
        if (dateInput && dateInput.value && !validateField(dateInput, 'date_naissance')) valid = false;
        if (pwdInput && !validateField(pwdInput, 'password')) valid = false;
        if (confirmPwd && pwdInput && !validateField(confirmPwd, 'password_confirm', pwdInput.value)) valid = false;

        const terms = form.querySelector('input[type="checkbox"]');
        if (terms && !terms.checked) {
            if (window.showToast) showToast('Vous devez accepter les conditions générales', 'warning');
            valid = false;
        }
    ...
    form.addEventListener('submit', function (e) {
        ...
        if (pwdInput) {
            if (!pwdInput.value.trim()) {
                showError(pwdInput, 'Le mot de passe est obligatoire');
                valid = false;
            }
        }
