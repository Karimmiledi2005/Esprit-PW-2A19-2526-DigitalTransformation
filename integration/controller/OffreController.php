...
{
    private string $baseUrl = BASE_URL . '/controller/OffreController.php';
    ...
    public function index(): void
    {
        ...
        try {
            $stmt = $db->query("SELECT * FROM offre ORDER BY date_creation DESC");
            $offres = $stmt->fetchAll(PDO::FETCH_ASSOC);
        } catch (Exception $e) {
            $erreur = 'Erreur lors du chargement des offres : ' . $e->getMessage();
        }
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (empty($errors)) {
                $offre = new Offre(
                    ...
                    (isset($_POST['plafond']) && $_POST['plafond'] !== '') ? round((float) $_POST['plafond'], 3) : null,
                    (int) $_POST['duree_min'],
                    $_POST['statut'] ?? 'active'
                );
    ...
    public function supprimer(): void
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            try {
                $check = $db->prepare("SELECT COUNT(*) FROM paiement WHERE id_offre = ?");
                $check->execute([$id]);
                $count = (int) $check->fetchColumn();

                if ($count > 0) {
                    $db->prepare("UPDATE offre SET statut = 'archivee' WHERE id_offre = ?")->execute([$id]);
                    $this->redirectToIndex('Offre archivée car elle possède des paiements liés');
                } else {
                    $db->prepare("DELETE FROM offre WHERE id_offre = ?")->execute([$id]);
    ...
    public function changerStatut(): void
    {
        ...
        $statutsValides = ['active', 'suspendue', 'archivee'];
        ...
        $db = config::getConnexion();
        $stmt = $db->prepare("SELECT id_offre FROM offre WHERE id_offre = ?");
        $stmt->execute([$id]);
        ...
        try {
            $db->prepare("UPDATE offre SET statut = ? WHERE id_offre = ?")->execute([$statut, $id]);
    ...
    private function valider(array $data): array
    {
        ...
        $typesValides = ['auto', 'sante', 'habitation', 'vie'];
        if ($type_offre === '' || !in_array($type_offre, $typesValides, true)) {
        ...
        if ($prix_mensuel === '' || !is_numeric($prix_mensuel) || (float)$prix_mensuel <= 0) {
            $errors['prix_mensuel'] = 'Le prix mensuel doit être valide.';
        }

        if ($prix_annuel === '' || !is_numeric($prix_annuel) || (float)$prix_annuel <= 0) {
            $errors['prix_annuel'] = 'Le prix annuel doit être valide.';
        }

        if ($plafond !== '' && (!is_numeric($plafond) || (float)$plafond <= 0)) {
            $errors['plafond'] = 'Le plafond doit être valide.';
        }

        if ($duree_min === '' || !ctype_digit($duree_min) || (int)$duree_min < 1) {
            $errors['duree_min'] = 'La durée minimale doit être un entier positif.';
        }
