<?php
/**
 * EmailService — Envoi des emails transactionnels sinistre
 *
 * Utilise PHPMailer (SMTP) si disponible, sinon repli sur mail() natif PHP.
 *
 * ─── Configuration ──────────────────────────────────────────────────────────
 * Modifiez les constantes SMTP_* ci-dessous avec vos identifiants.
 * Pour Gmail : activez "Mots de passe d'application" dans votre compte Google.
 * ────────────────────────────────────────────────────────────────────────────
 */

// ── Chargement PHPMailer (Composer ou inclusion manuelle) ──────────────────
...
class EmailService
{
    // ── ⚙️  CONFIGURATION SMTP — À adapter ───────────────────────────────────
    private const SMTP_HOST       = 'smtp.gmail.com';   // ex: smtp.mailtrap.io
    private const SMTP_PORT       = 587;                 // 587=STARTTLS, 465=SSL
    private const SMTP_USER       = 'muledikarim@gmail.com';
    private const SMTP_PASS       = 'tpgpkkqwvnypvovj';
    private const FROM_EMAIL      = 'muledikarim@gmail.com';
    private const FROM_NAME       = 'Assurance — Service Sinistres';
    // ─────────────────────────────────────────────────────────────────────────
    ...
    private function send(string $toEmail, string $toName, string $subject, string $body, ?string $pdfContent = null, ?string $pdfName = null): bool
    {
        ...
        // ── Fallback : mail() natif (ATTENTION : pas de support PDF ici) ────
        if ($pdfContent) {
