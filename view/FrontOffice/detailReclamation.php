    ...
    <main class="main">
        ...
        <div class="form-page-card">
            ...
            <div style="margin-bottom:16px">
                <div style="font-size:12px;font-weight:600;color:var(--text-muted,#64748b);text-transform:uppercase;letter-spacing:.05em;margin-bottom:6px;display:flex;align-items:center;gap:10px">
                    Description
                    <button id="btnTranslateDesc" class="btn-translate" title="Translate to English">
                        <i class="bi bi-translate"></i> EN
                    </button>
                </div>
                <div class="rec-desc" id="recDescText" style="margin:0"><?php echo h($row['description']); ?></div>
            </div>
            <?php endif; ?>
        </div>
        ...
        <div class="form-page-card" style="margin-top:16px;border-left:4px solid var(--accent,#6366f1)">
            ...
            <div style="background:var(--surface2,#f8fafc);border-radius:8px;padding:16px;font-size:14px;line-height:1.7;color:var(--text-main,#334155)">
                <div style="display:flex;justify-content:flex-end;margin-bottom:8px">
                    <button id="btnTranslateRep" class="btn-translate" title="Translate to English">
                        <i class="bi bi-translate"></i> EN
                    </button>
                </div>
                <div id="repContenuText"><?php echo nl2br(h($row['reponse_contenu'] ?? '')); ?></div>
            </div>
        </div>
        <?php else: ?>
...
.btn-translate {
  ...
  color: #6366f1; cursor: pointer; transition: all .2s;
  white-space: nowrap;
}
.btn-translate:hover   { background: rgba(99,102,241,.2); transform: translateY(-1px); }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const btnDesc = document.getElementById('btnTranslateDesc');
  const descEl  = document.getElementById('recDescText');
  if (btnDesc && descEl) attachTranslateToggle(btnDesc, descEl);

  /* ── Réponse de l'administration ── */
  const btnRep = document.getElementById('btnTranslateRep');
  const repEl  = document.getElementById('repContenuText');
