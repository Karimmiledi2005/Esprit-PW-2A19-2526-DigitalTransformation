...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add') {
    try {
        ...
        $refContrat  = trim($_POST['ref_contrat'] ?? 'REF-0000');
        ...
        if ($objet && $description && $email) {
            ...
            // On retourne toujours un message clair pour le fetch
            echo "success";
            exit;
        }
    } catch (Exception $e) {
                ...
                <div class="avatar-dropdown" id="avatarDropdown">
                    <div class="dropdown-header">
                        ...
                        <div class="dropdown-info">
                            <div class="dropdown-name"><?php echo h($_SESSION['prenom']??'').' '.h($_SESSION['nom']??''); ?></div>
                            <div class="dropdown-email"><?php echo h($_SESSION['email']??''); ?></div>
                        ...
                        <div class="rec-header">
                            <div class="rec-title-group">
                                ...
                                <div>
                                    ...
                                    <div class="rec-ref"> <?php echo h($reclamation['rec_ref'] ?? ''); ?> &nbsp;·&nbsp; Contrat : <?php echo h($reclamation['ref_contrat'] ?? ''); ?> </div>
                                </div>
                            </div>
        ...
        <div id="reclamationModal" class="modal-overlay">
            <div class="modal-box">
                ...
                <form id="reclamationModalForm" onsubmit="handleReclamationSubmit(event)">
                    ...
                    <!-- Référence Contrat -->
                    <div class="animated-field">
                        <label><i class="bi bi-file-earmark-text"></i> RÉFÉRENCE CONTRAT *</label>
                        <select id="fRefContrat" name="ref_contrat" class="form-control" required>
...
<script>
    ...
    function loadContractsForModal() {
        ...
        fetch('api_get_user_contracts.php')
            ...
            .then(d => {
                ...
                if (d.success && d.contracts) {
                    while (select.options.length > 1) select.remove(1);
                    d.contracts.forEach(c => {
                        const opt = document.createElement('option');
                        opt.value = c.numero_contrat;
                        opt.textContent = c.numero_contrat + ' (' + c.type_contrat + ')';
                        select.appendChild(opt);
                    });
                }
            })
            .catch(e => {
                console.error('Erreur contrats:', e);
                loading.style.display = 'none';
            });
    }
    ...
    function handleReclamationSubmit(e) {
        ...
        const refContrat  = document.getElementById('fRefContrat').value.trim();
        ...
        formData.append('ref_contrat', refContrat);
